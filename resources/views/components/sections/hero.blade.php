        ...
        <div class="w-full lg:w-1/2 self-stretch {{ $bgColorClass }} pt-8 pb-16 px-5 lg:p-10 lg:py-24 flex flex-col {{ $contentAlignment }} gap-6 lg:gap-10
          ...
          <div class="flex flex-col {{ $contentAlignment }} gap-6 lg:gap-10 max-w-3xl">
            ...
            <div class="flex flex-col gap-2 lg:gap-3 w-full">
                @if($manual)
                    @if($heroKicker)
                        <h1 class="font-display text-[48px] leading-[1] tracking-[-0.04em] lg:text-[88px] uppercase text-blue-black">{{ $heroKicker }}</h1>
                    @endif
                    @if($heroTitle)
                        <p class="font-display text-[24px] leading-[1.1] tracking-[-0.04em] lg:text-[41px] text-blue-black">{{ $heroTitle }}</p>
                    @endif
                @else
            ...
            {{-- Button --}}
            <a href="{{ $heroUrl }}" class="btn btn-{{ $buttonColor }}" aria-label="{{ $buttonText }}: {{ $heroKicker ?: $heroTitle }}">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M12 5V19M5 12H19" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
                <span>{{ $buttonText }}<span class="sr-only">: {{ $heroKicker ?: $heroTitle }}</span></span>
            </a>
          </div>
        </div>
    </div>
