...
<section class="feature-2-wrapper">
    ...
    >
        ...
        <div class="feature-2-content pt-[100px] lg:pt-0 {{ $bgColorClass }}" @if($bgColorStyle) style="{{ $bgColorStyle }}" @endif>
            <div class="flex flex-col items-center justify-start lg:justify-center text-center gap-10 max-lg:gap-6 w-full h-full max-w-[100vw] px-10 max-lg:px-5 max-lg:pb-20 lg:-mt-[100px]">
                ...
                @if($buttonText && $buttonUrl && $buttonUrl !== '#')
                    <a href="{{ $buttonUrl }}" class="btn btn-{{ $buttonVariant }}" aria-label="{{ $buttonText }}: {{ $kicker ?: $title }}">
                        ...
                        <span>{{ $buttonText }}<span class="sr-only">: {{ $kicker ?: $title }}</span></span>
                    </a>
                @endif
            </div>
        </div>
