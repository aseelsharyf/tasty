        ...
        <div class="w-full max-w-[1440px] mx-auto px-10 pt-16 pb-32 max-lg:px-5 max-lg:pt-10 max-lg:pb-16 flex flex-col gap-16 max-lg:gap-10">
            ...
            <div class="flex flex-col gap-5 items-center text-center text-blue-black w-full max-w-[660px] mx-auto">
                ...
                <form action="{{ route('products.index') }}" method="GET" class="w-full max-w-sm mt-2">
                    <div class="relative">
                        ...
                        />
                        @if(!empty($search))
                            <a href="{{ route('products.index') }}" class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                    ...
                    <div class="px-5 lg:px-0 overflow-x-auto lg:overflow-visible scrollbar-hide" x-ref="scroller" @scroll="check()" x-init="check()">
                        <div class="flex lg:flex-wrap lg:justify-center gap-2 lg:gap-3 w-max lg:w-auto px-3 lg:px-0">
                            <a
                                ...
                                class="px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-colors {{ !$currentCategory ? 'bg-blue-black text-white' : 'bg-off-white text-blue-black hover:bg-gray-200' }}"
                            >
                                All
                            </a>
            ...
            @else
                <div class="text-center text-gray-500 py-16">
                    <p class="text-body-lg">{{ !empty($search) ? "No products found for \"{$search}\"." : 'No products found.' }}</p>
                </div>
            @endif
        </div>
    </section>
