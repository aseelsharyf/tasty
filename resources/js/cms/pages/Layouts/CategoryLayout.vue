...
// Context for filtering posts by this category
...
function addSection(type: string, position?: number) {
    ...
    const newSection: HomepageSection = {
        ...
        dataSource: {
            ...
            // Default to filtering by this category
            action: 'byCategory',
            params: { slugs: [props.category.slug] },
        },
...
function saveChanges() {
    isSaving.value = true;
    form.put(cmsPath(`/layouts/categories/${props.category.uuid}`), {
        preserveScroll: true,
        onSuccess: () => {
            toast.add({ title: 'Saved', description: 'Category layout updated successfully.', color: 'success' });
            isSaving.value = false;
        },
        onError: (errors) => {
            toast.add({ title: 'Error', description: 'Failed to save category layout.', color: 'error' });
            console.error('Category layout save error:', errors);
            isSaving.value = false;
        },
    });
}
...
<template>
    <Head :title="`${category.name} Layout`" />

    <DashboardLayout>
        <UDashboardPanel id="layouts-category">
            <template #header>
                <UDashboardNavbar>
                    <template #leading>
                ...
                <div class="flex flex-col gap-6 max-w-7xl mx-auto py-6 px-4 lg:px-6">
                    ...
                    <div class="flex items-start justify-between gap-4">
                        <div>
                            ...
                            <p class="text-sm text-muted mt-1">
                                Configure the layout for the <strong>{{ category.name }}</strong> category page.
                            </p>
                        </div>
                    </div>
                                ...
                                <div>
                                    ...
                                    <div class="text-sm text-muted">
                                        {{ form.useCustomLayout
                                            ? 'Using a custom section-based layout for this category.'
                                            : 'Using the default category template with paginated posts.'
                                        }}
                                    </div>
                                </div>
                            </div>
                    ...
                    <template v-if="form.useCustomLayout">
                        ...
                        >
                            ...
                            <p class="text-sm text-muted mt-1 mb-4">
                                Add sections to build the {{ category.name }} category page layout.
                            </p>
                            <UButton
                                icon="i-lucide-plus"
                                @click="showAddSectionModal = true"
                    ...
                    <div v-else class="rounded-xl border border-default bg-elevated/30 p-8 text-center">
                        ...
                        <p class="text-sm text-muted mt-2 max-w-md mx-auto">
                            This category page will use the default template which displays a paginated list of posts.
